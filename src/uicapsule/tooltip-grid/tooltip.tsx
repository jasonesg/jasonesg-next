  ...
  return (
    <FloatingPortal>
      ...
      {blockType && (
        <AnimatePresence>
          {context.open && (
            <div className="tooltip-lines-cage">
              <TooltipLines context={context} />
            </div>
          )}
        </AnimatePresence>
      )}
...
const TooltipLines = ({ context }: { context: ContextType }) => {
  ...
  React.useLayoutEffect(() => {
    if (!context?.open || !floatingEl) return;
    ...
    // Polling as a fallback for initial mounting animation
  ...
  }, [floatingEl, context?.open]);

  if (!floatingEl || !context || coords.width === 0) return null;

  return (
    <>
      <motion.div
        className={`${"line"} ${"lineH"}`}
        initial={{ opacity: 0, top: coords.y - 20 }}
        animate={{ opacity: 1, top: coords.y }}
        exit={{ opacity: 0, top: coords.y - 20 }}
        transition={{
          ease: easeInOutQuint,
          duration: 0.3,
        }}
      ...
      <motion.div
        className={`${"line"} ${"lineH"}`}
        initial={{ opacity: 0, top: coords.y + coords.height + 20 }}
        animate={{ opacity: 1, top: coords.y + coords.height }}
        exit={{ opacity: 0, top: coords.y + coords.height + 20 }}
        transition={{
          ease: easeInOutQuint,
          duration: 0.3,
        }}
      ...
      <motion.div
        ...
        style={{ height: "200dvh" }}
        initial={{ opacity: 0, left: coords.x - 20 }}
        animate={{ opacity: 1, left: coords.x }}
        exit={{ opacity: 0, left: coords.x - 20 }}
        transition={{
          ease: easeInOutQuint,
          duration: 0.3,
        }}
      ...
      <motion.div
        className={`${"line"} ${"lineV"}`}
        style={{ height: "200dvh" }}
        initial={{ opacity: 0, left: coords.x + coords.width + 20 }}
        animate={{ opacity: 1, left: coords.x + coords.width }}
        exit={{ opacity: 0, left: coords.x + coords.width + 20 }}
        transition={{
          ease: easeInOutQuint,
          duration: 0.3,
        }}
